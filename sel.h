...
void createLocalI(Matrix &I,mesh m){
    float DELTA = m.getParameter(DELTA_S);
...
void createLocalP(Matrix &P,mesh m){
    ...
    float KAPPA = m.getParameter(KAPPA_K);
...
void createLocalQ(Matrix &Q,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float IPSILON = m.getParameter(IPSILON_U);
    Q.at(0).at(0) += IPSILON/l;      Q.at(0).at(1) += -IPSILON/l;
    Q.at(1).at(0) += -IPSILON/l;     Q.at(1).at(1) += IPSILON/l;
}
